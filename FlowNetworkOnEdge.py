...
class FlowNetwork:
    ...
    def getJunction(self, node, vec):
        ...
        """
        junction = {'in': [], 'out': []}
        for component in self.graph.in_edges(node[0]):
            direction = 'in'
        ...
        for component in self.graph.out_edges(node[0]):
            direction = 'out'
            q = self.getval((component[0], component[1]), vec, 'q')
            if q < 0:
                direction = 'in'
    ...
    def getunknowns(self, vector=False):
        ...
        Args:
            vector (bool): should the initial value vector be returned?
    """
    ...
    for key, value in self.nodeVariables.iteritems():
        ...
        for node in self.getnodes(True):
            if key in node[1]:
                sum += np.abs(node[1][key])
                n += 1;
    if n > 0:
        self.nodeVariables[key] = sum / n;
    # print self.nodeVariables
    for node in self.getnodes(True):
    ...
    for key, value in self.componentVariables.iteritems():
        ...
        for component in self.getcomponents(True):
            ...
            # vector entry
            if key in component[2]:
                sum += np.abs(component[2][key])
                n += 1;
    if n > 0:
        self.componentVariables[key] = sum / n;
    elif key is 'm':
        self.componentVariables[key] = self.nodeVariables['s'];
    # print self.componentVariables
    for component in self.getcomponents(True):
        # For every component check if all keys have a value, if not create a
        # vector entry
        for key, value in self.componentVariables.iteritems():
...
def getresult(self, vec):
    result = deepcopy(self)

    if self.translation is {}:
        print ('Error: network unknowns are unknown.')
        return False
    for node in self.translation['nodes']:
        for (k, v) in self.translation['nodes'][node].iteritems():
            # print node
            # print k
            result.addnodes((node, {k: self.getval(node, vec, k)}))
    for component in self.translation['components']:
        for (k, v) in self.translation['components'][component].iteritems():
            result.addcomponents([(component[0], component[1], {k: self.getval((component[0], component[1]), v)})])
    return result
